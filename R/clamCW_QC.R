#' clamCW_QC
#'
#' @param cwFolder  default is \code{NULL}
...
clamCW_QC <- function(cwFolder = NULL, ...){
  ...
  Commercial_Sample_Profile <- df_list[["Commercial_Sample_Profile"]]
  Length_Frequency_Sample <- df_list[["Length_Frequency_Sample"]]
  Length_Frequency <- df_list[["Length_Frequency"]]
  ...
  grouped_df <- dplyr::group_by(combined_df, .data$`LATITUDE INIT`, .data$`LONGITUDE INIT`)

  count_df <- dplyr::summarise(grouped_df, num_sources = dplyr::n_distinct(.data$source))
  differing_records <- dplyr::filter(count_df, .data$num_sources == 1)
  differing_df <- dplyr::inner_join(differing_records, combined_df, by = c("LATITUDE INIT", "LONGITUDE INIT"))  
  if(nrow(differing_records)>0){
    differing_records_output <- utils::capture.output(utils::write.table(differing_records, sep = "\t", row.names = FALSE, quote = FALSE))
    ...
    differentWt_output <- utils::capture.output(utils::write.table(differing_records_output, sep = "\t", row.names = FALSE, quote = FALSE))
  ...
  combinations <- utils::combn(seq_along(df_list), 2)
  for(i in seq(ncol(combinations))) {
    ...
    ##############
    pair <- combinations[,i]
    df1 <- df_list[[cw_fileTypes[pair[1]]]]$chkDf
    df2 <- df_list[[cw_fileTypes[pair[2]]]]$chkDf
  ...
  recProd[["Record"]] <- df_list[["Record"]]$chkDf
  recProd[["Product"]] <- df_list[["Product"]]$chkDf
  return(recProd)
}